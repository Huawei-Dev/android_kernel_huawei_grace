...
static long hifi_get_dmesg(unsigned long arg)
{
	...
	ret = simple_read_from_buffer(dump_info.user_buf_l, dump_info.buf_size, &pos,
			g_dsp_dump_info[0].data_addr, len);
	if (ret != len) {
		loge("copy to user fail, ret is %d\n", ret);
		len = ret;
	}
	...
	if (0 == dump_info.buf_size)
	{
		*g_misc_data.dsp_log_cur_addr = DRV_DSP_UART_TO_MEM + DRV_DSP_UART_TO_MEM_RESERVE_SIZE;
		if (g_dsp_dump_info[0].data_len > DRV_DSP_UART_TO_MEM_SIZE) {
			loge("data_len is larger than DRV_DSP_UART_TO_MEM_SIZE\n");
			len = 0;
		} else {
...
static void hifi_dump_dsp(DUMP_DSP_INDEX index)
{
	...
	fp = filp_open(file_name, file_flag, 0640);
	if (IS_ERR(fp)) {
...
void hifi_write_file_from_memory_regions(char *filename, u32 addr, unsigned int size)
{
	...
	fp = filp_open(filename, file_flag, 0640);
	if (IS_ERR(fp)) {
...
int hifi_dsp_write_param(unsigned long arg)
{
	...
	if (para.para_size_in > SIZE_PARAM_PRIV) {
									  ...
									  struct device_attribute *attr, char __user *buf)
{
	...
	return simple_read_from_buffer(buf, PAGE_SIZE, &pos, level_str, strlen(level_str) + 1);
}
		...
		if (recv) {
			...
			if (unlikely(len >= PAGE_MAX_SIZE)) {
				...
				if (0 != len) {
				    ...
				    ret = recv->rev_msg.mail_buff_len - SIZE_CMD_ID - len;
