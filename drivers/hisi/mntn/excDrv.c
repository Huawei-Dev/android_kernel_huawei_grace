...
static int exch_get_a_core_base_addr(void)
{
    ...
    printk("g_exch_a_core_base_addr: phy[0x%x], vir[%pK], size[0x%x]\n", ANDROID_DUMP_LOG_ADDR,
...
void exch_stand_guard(struct task_struct *tsk)
{
	...
	{
		printk(KERN_ERR
			"exch_stand_guard, tsk addr [0x%pK], init_mm.pgd addr [0x%pK]!\n",
			tsk, init_mm.pgd);
		while(1){
			cpu_relax();
