...
wp_hbp_register(void *ibp_addr, unsigned int ibp_len, unsigned int ibp_type)
{
	...
	mlog_i("Begin, addr=0x%pK, len=%u, type=%d\n", ibp_addr, ibp_len, ibp_type);
	...
	if (IS_ERR((void __force *)pe)) {
		mlog_e("register failed, addr=0x%pK\n", ibp_addr);
		ret = -1;
		...
		if (idx >= 0) {
			...
			mlog_e("find space failed, addr=0x%pK, idx=%d\n", ibp_addr, idx);
			ret = -2;
		}
...
static void wp_hbp_unregister(void *ibp_addr)
{
	...
	mlog_i("Begin, addr=0x%pK", ibp_addr);
	...
	if (idx >= 0) {
		...
		mlog_e("error, addr=0x%pK", ibp_addr);
	}
...
static void wp_state_dp(char *buf)
{
	...
	for (i = 0; i < ARM_MAX_BRP; i++) {
	    ...
	    snprintf(buf, WP_DBG_SIZE_PAGE, "%s[%d] addr = 0x%pK\n", buf, i, wp_info[i].wp_addr);
	    ...
	    snprintf(buf, WP_DBG_SIZE_PAGE, "%s[%d] pe = 0x%pK\n", buf, i, wp_info[i].wp_hbp);
	}
...
static ssize_t wp_config_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
    ...
    if ('s' == kn_buf[0]) {
	    ...
	    mlog_e("sscanf addr=0x%pK, num=%d", addr, num);
        if (1 == num) {
            if (addr != 0) {
                      ...
                      size_t count, loff_t *ppos)
{
    ...
    /* context */
    snprintf(kn_buf, WP_DBG_SIZE_PAGE, "addr got = 0x%pK, size=0x%08x\n", tget_mem_addr, tget_mem_size);
                      ...
                      size_t count, loff_t *ppos)
{
    ...
    /* context */
    if (tget_mem_addr != 0) {
        snprintf(kn_buf, WP_DBG_SIZE_PAGE, "addr(0x%pK) = 0x%x\n", tget_mem_addr, tacc_value);
        snprintf(kn_buf, WP_DBG_SIZE_PAGE, "%s\n", kn_buf);
    }
    snprintf(kn_buf, WP_DBG_SIZE_PAGE, "%susage:\n", kn_buf);
...
static int __init hw_break_module_init(void)
{
	...
	if (wp_hbp_register(pe_addr, HW_BREAKPOINT_LEN_4, HW_BREAKPOINT_W | HW_BREAKPOINT_R)) {
        mlog_e("watchpoint register failed, addr=0x%pK\n", pe_addr);
        ret = -EFAULT;
		goto fail;
	}
#endif
