        ...
        struct kref* r)
{
    ...
    HWCAM_CFG_INFO("instance(0x%pK) type=%ld, port=%d", stm, stm->stream_type, stm->port.id);
        ...
        hwisp_buf_t* buf)
{
	...
	if ((buf->info.ion_fd <= 0) || IS_ERR_OR_NULL(stm->ion_vc)) {
		HWCAM_CFG_ERR("invalid ion: fd=%d, client=%pK",
        ...
        struct list_head* streams)
{
    ...
    HWCAM_CFG_INFO("instance(0x%pK) portid=%u", stm, stm->port.id);
