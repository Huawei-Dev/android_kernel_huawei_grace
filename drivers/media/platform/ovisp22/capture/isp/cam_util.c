...
inline void del_from_queue(camera_frame_buf *f, u32 flag)
{
	if (f->flags & flag) {
		...
		print_warn("frame[%pK] NOT in queue with flag[%d]", (void*)f->phyaddr, flag);
	}
}
...
inline void add_to_queue(camera_frame_buf *f, struct list_head *l, u32 flag)
{
	if (!(f->flags & flag)) {
		...
		print_warn("frame[%pK] has been added to queue with flag[%d]", (void*)f->phyaddr, flag);
	}
}
/*
 **************************************************************************
...
void camera_free_ion_handle(struct ion_client *ion_client, buffer_arr_t *pbuf)
{
    ...
    if((NULL == ion_client) || (NULL == pbuf)){
        print_error("%s:ion_client=%pK,pbuf=%pK.", __func__,ion_client,pbuf);
        return;
    }
	...
	if(pbuf->buffers[i].viraddr != NULL){
            ...
            print_info("unmap kernel handle=%pK success",pbuf->buffers[i].ion_handle);
            pbuf->buffers[i].viraddr = NULL;
        }

        ion_unmap_iommu(ion_client, pbuf->buffers[i].ion_handle);
        print_info("unmap iommu handle=%pK success",pbuf->buffers[i].ion_handle);
        if(pbuf->buffers[i].ion_handle != NULL){
            print_info("%s:ion_client=%pK,i=%d,handle=%pK.", __func__,ion_client,i,pbuf->buffers[i].ion_handle);
            ion_free(ion_client, pbuf->buffers[i].ion_handle);
            print_info("ion free handle=%pK success",pbuf->buffers[i].ion_handle);
