	...
	case DBG_INFO_ISP_BASE:
		{
			...
			print_info("cam_dbg_info.dbg_isp_base_addr=%pK", cam_dbg_info.dbg_isp_base_addr);
			break;
		}
	case DBG_INFO_CSI0_BASE:
		{
			cam_dbg_info.dbg_csi0_base_addr = (u8*) context;
			print_info("cam_dbg_info.dbg_csi0_base_addr=%pK", cam_dbg_info.dbg_csi0_base_addr);
			break;
		}
	case DBG_INFO_CSI1_BASE:
		{
			cam_dbg_info.dbg_csi1_base_addr = (u8*) context;
			print_info("cam_dbg_info.dbg_csi1_base_addr=%pK", cam_dbg_info.dbg_csi1_base_addr);
			break;
		}
	default:
		{
...
void dump_queue(struct list_head *list_queue, u32 flag)
{
	...
	list_for_each(pos, list_queue) {
		...
		if (frame)
			print_info("phyaddr:%pK, index:%d", (void*)frame->phyaddr, frame->index);
	}
#endif
}
/*
