				      ...
				      struct v4l2_buffer *b, struct ion_client *ion_client, int share_fd)
{
	...
	if (IS_ERR(a->ion_handle) || a->ion_handle == NULL) {
		print_error("%s, ion_import_dma_buf fail.ion_handle=%pK.ion_client=%pK,a->share_fd=%d.\n",
					__func__, a->ion_handle, ion_client, a->share_fd);
		return;
	}
	print_info("%s, frame ion_handle=%pK", __func__, a->ion_handle);
	...
	print_debug("%s: frame phyaddr=%pK, viraddr=%pK", __func__, (void*)a->phyaddr, a->viraddr);
}
				     ...
				     struct v4l2_ext_controls *arg)
{
	...
	/* allocate buffer */
	print_debug("user_ptr 0x%pK, count %d", ext_ctls->controls, ext_ctls->count);
	controls = (struct v4l2_ext_control *)ext_ctls->controls;
			      ...
			      struct v4l2_buffer *b)
{
	...
	print_debug("enter %s, type=%d, index=%d, phyaddr=%pK, size=%d, mem[%d]",
			__func__, b->type, b->index, (void*)b->m.offset, b->length, b->memory);
	...
	{
	    print_info("reserved buf:state=%d,index=%d,phyaddr=0x%pK",STATE_PREVIEW, CAMERA_BUF_RSV_BUF_IDX,cam->buffer_arr[state].buffers[b->index].phyaddr);
	    return 0;
	}
			       ...
			       struct v4l2_buffer *b)
{
    ...
    ***************************************************************************/
	...
	print_debug("exit %s, state=%d, index=%d, phyaddr is 0x%pK", __func__, state,
...
static int k3_v4l2_open(struct file *file)
{
	...
	if (0 == cam->open_count++) {
	    ...
	    print_info("ion_dev=%pK,ion_client=%pK",ion_dev,cam->ion_client);
	} else {
...
static int k3_init_v4l2_ctl(v4l2_ctl_struct *v4l2_ctl)
{
	...
	print_debug("enter %s, cam %pK", __func__,  v4l2_ctl);
...
static int sensor_otp_proc_show(struct seq_file *seq, void *v)
{
	...
	{
		print_error("sensor or check_otp_status is NULL! sensor:%pK", v4l2_ctl.sensor);
		return -EFAULT;
	}
	...
	const char __user *ubuf, size_t cnt, loff_t * ppos)
{
	...
	if ( 2 == sscanf(buf, "%x %x", &reg, &value))//write reg
	{
		...
		{
			print_error("sensor or set_sensor_reg is NULL! sensor:%pK", v4l2_ctl.sensor);
			return -EFAULT;
		}
	...
	size_t count, loff_t *ppos)
{
	...
	{
		print_error("sensor or get sensor_reg is NULL! sensor:%pK", v4l2_ctl.sensor);
		return -EFAULT;
	}
