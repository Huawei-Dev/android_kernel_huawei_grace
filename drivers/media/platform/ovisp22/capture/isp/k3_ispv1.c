...
static void append_meta_data_to_raw_imgdata(camera_frame_buf* frame)
{
    ...
    print_info("%s, viraddr=0x%pK", __FUNCTION__, frame->viraddr);
    // append info;
    base = (char*)frame->viraddr;
    pos = base + info.img_width*info.img_height*2;
    print_info("%s, viraddr offset=0x%pK", __FUNCTION__, pos);
...
static int ispv1_offline_cmd(pic_attr_t *pic_attr, u32 source_addr, u32 target_addr)
{
	...
	print_info("%s, in_width=%d, in_height=%d, out_width=%d, out_height=%d, "
		" out_stride=%d\n, source_addr=%pK, target_addr=%pK, offline_state=%d",
		__func__, pic_attr->in_width, pic_attr->in_height,
		pic_attr->out_width, pic_attr->out_height,
		pic_attr->out_stride, (void*)source_addr, (void*)target_addr, isp_hw_data.offline_state);
	...
	camera_sensor *sensor, bool flash_on, camera_scene scene)
{
	...
	print_info("%s, in_width=%d, in_height=%d, out_width=%d, out_height=%d,"
			" out_stride=%d, phyaddr=%pK, count=%d, bypass=%d, sensor setting:%d",
			__func__, pic_attr->in_width, pic_attr->in_height,
			pic_attr->out_width, pic_attr->out_height, pic_attr->out_stride,
			(void*)phyaddr, count, is_bypass_isp, frame_index);
                ...
                u32             target_addr)
{
    ...
    print_info("enter %s.target_addr=%pK,zsl_proc_idx=%d",__func__,(void*)target_addr,this_ispdata->zsl_ctrl.zsl_proc_idx);
...
static int ispv1_start_proc_img(pic_attr_t *pic_attr, ipp_mode mode, buffer_arr_t *ipp_buf_arr)
{
    ...
    if (CAMERA_ZSL_OFFLINE == this_ispdata->zsl_ctrl.zsl_proc_mode){
        ...
        print_debug("%s:framedst->phyaddr=%pK,dst_viraddr=%pK", __func__,(void*)framedst->phyaddr,(void*)dst_viraddr);
        ...
        print_debug("%s:framesrc->phyaddr=%pK,src_viraddr=%pK", __func__,(void*)framesrc->phyaddr,(void*)src_viraddr);
...
static int ispv1_start_process(pic_attr_t *pic_attr, u8 mode, buffer_arr_t *ipp_buf_arr)
{
	...
	if (mode == CAMERA_IPP_MODE_HDR) {
		...
		print_debug("****framesrc=0x%pK, idx=%d, flag=%d, phy=%pK****",
			framesrc, framesrc->index, framesrc->flags, (void*)framesrc->phyaddr);
	...
	u32 size, i2c_length length, u8 mask, i2c_length addr_len)
{
	...
	do {
		...
		print_debug("write_seq_thr_isp buf[%pK], i2c_writen=%d, i2c_write_num=%d",
			buf, i2c_writen, i2c_write_num);
...
static int ispv1_hw_init(struct platform_device *pdev, data_queue_t* data_queue)
{
	...
	print_debug("startisp load firmware OK basereg[%pK]\n", (void*)isp_hw_data.base);
	return 0;
...
static int ispv1_switch_sensor_cmd(camera_sensor *sensor, stream_state state)
{
	...
	*/
	...
	print_info("stream_setting_size = %d , stream_setting = %pK",stream_setting_size, (void*)stream_setting);
